...
data class ChatMessage(
    ...
    val isButton: Boolean = false //버튼 관련 필드
)
...
class ChatAdapter(private val messages: MutableList<ChatMessage>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    inner class ButtonViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val button: Button = itemView.findViewById(R.id.addPlaceButton)
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (holder) {
            ...
            is ButtonViewHolder -> {
                holder.button.text = "${chatMessage.message} 추가" //관광지 추가 버튼 생성
                holder.button.setOnClickListener {
                    //버튼 클릭 시 onButtonClickListener 호출하여 DB에 관광지 데이터 추가
